...
export class UserInfrastructure implements UserRepository {
    ...
    async getAll(): Promise<UserCreateResponse | UserCreateResponse[]> {
        const repository = DatabaseBootstrap.dataSource?.getRepository(UserEntity);
        const userEntities = await repository?.find({where: {active: true}});

        return UserModelDto.fromDataToResponse(userEntities);
    }
    async getByPage(page: number, pageSize: number): Promise<[entities: UserCreateResponse[], total:number]> {
        const repository = DatabaseBootstrap.dataSource?.getRepository(UserEntity);
        const [userEntities, total] = await repository?.findAndCount({skip: page*pageSize, take: pageSize, where: {active: true}});

        const entities = UserModelDto.fromDataToResponse(userEntities) as UserCreateResponse[]
