...
export class UserModelDto {
    ...
    static fromDataToResponse(data: UserEntity | UserEntity[]): UserCreateResponse | UserCreateResponse[] {
        if (Array.isArray(data)) {
            return data.map((item) => this.fromDataToResponse(item)) as UserCreateResponse[];
        }
        return {
            id: data.id,
            name: data.name,
