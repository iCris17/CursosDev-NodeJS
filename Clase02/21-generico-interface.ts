...
class MedicInfrastructure implements IRepository<Medic, string> /*IRepositoryMedic*/ {
    save(medic: Medic): Medic{
        return medic
    }
    update(id: string, medic: Medic): Medic{
...
class UserInfrastructure implements IRepository<User, number> /*IRepositoryUser*/{
    save(user: User): User {
        return user
    }
    update(id: number,user: User): User {
